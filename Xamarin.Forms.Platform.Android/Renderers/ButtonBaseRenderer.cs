...
{
	public class ButtonBaseRenderer<TButton, TAButton> : ViewRenderer<TButton, TAButton>, AView.IOnAttachStateChangeListener, IBorderVisualElementRenderer
		where TButton : Button, IElementConfiguration<TButton>
		where TAButton : AButton
	{
		...
		public ButtonBaseRenderer(Context context) : base(context)
		{
			AutoPackage = false;
		...
		[Obsolete("This constructor is obsolete as of version 2.5. Please use ButtonBaseRenderer(Context) instead.")]
		...
		class ButtonClickListener : Object, IOnClickListener
		{
			...
			{
				var renderer = v.Tag as ButtonBaseRenderer<TButton, TAButton>;
				if (renderer != null)
